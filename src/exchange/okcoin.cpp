...
namespace OKCoin {
    ...
    double getAvail(Parameters &params, std::string currency) {
        ...
        if (currency.compare("usd") == 0) {
            returnedText = json_string_value(
                    json_object_get(json_object_get(json_object_get(json_object_get(root, "info"), "funds"), "free"),
                                    "usd"));
    ...
    int sendLongOrder(Parameters &params, std::string direction, double quantity, double price) {
        // signature
        std::ostringstream oss;
        oss << "amount=" << quantity << "&api_key=" << params.okcoinApi << "&price=" << price << "&symbol=btc_usd&type="
            << direction << "&secret_key=" << params.okcoinSecret;
        std::string signature = oss.str();
        oss.clear();
        oss.str("");
        // content
        oss << "amount=" << quantity << "&api_key=" << params.okcoinApi << "&price=" << price << "&symbol=btc_usd&type="
            << direction;
        std::string content = oss.str();
        *params.logFile << "<OKCoin> Trying to send a \"" << direction << "\" limit order: " << quantity << "@$"
                        << price << "..." << std::endl;
    ...
    bool isOrderComplete(Parameters &params, int orderId) {
        ...
        // signature
        std::ostringstream oss;
        oss << "api_key=" << params.okcoinApi << "&order_id=" << orderId << "&symbol=btc_usd" << "&secret_key="
            << params.okcoinSecret;
        std::string signature = oss.str();
        oss.clear();
        oss.str("");
    ...
    double getLimitPrice(Parameters &params, double volume, bool isBid) {
        ...
        double limPrice = 0.0;
        if (isBid) {
            root = json_object_get(getJsonFromUrl(params, "https://www.okcoin.com/api/v1/depth.do", "", GETRequest),
                                   "bids");
            // loop on volume
    ...
    int borrowBtc(Parameters &params, double amount) {
        ...
        std::ostringstream oss;
        oss << "api_key=" << params.okcoinApi << "&symbol=btc_usd&days=fifteen&amount=" << 1
